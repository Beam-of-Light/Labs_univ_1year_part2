...
void message_base::push_back(const data_msg& new_message)
{
	list.push_back(new_message);
}
...
void message_base::counting_sort()
{
	int arr[5] = {};
	for (data_msg current : list)
	...
	std::vector<data_msg> temp = list;
	for (int i = list.size() - 1; i >= 0; --i)
	{
		list[arr[temp[i].type]] = temp[i];
		--arr[temp[i].type];
	}
}

void message_base::radix_sort()
{
	for (size_t i = 0, exp = 1; i < 3; i++, exp *= 10)
	{
		int arr[10] = {};
		for (data_msg current : list)
		{
...
(const data_msg::date_time & date1, const data_msg::date_time & date2) const
{
	...
	for (data_msg current : list)
	{
		if (current.date < date2 && current.date > date1)
		{
